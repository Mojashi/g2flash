...
s.onRawFrame((f: any) => { if (!f.ok || f.sid !== SID) return; const p = f.pb; if (p[0] !== 0xa7) return;
  const op = String.fromCharCode(p[1]); const val = u32at(p, 2);
  console.log(`  <- ${op} = 0x${val.toString(16)} (${val})`); });
...
console.log("\nsending 'i' (IMU enable with gyro)...");
