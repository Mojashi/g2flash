...
async function requestCapture(session: G2Session): Promise<void> {
  // RT_OP_SEND(0x03) mode_id=1 data='s' -> payload sc_data -> cfw_screenshot_run() streams on 0x7d.
  const pb = new Uint8Array([0x03, 0x01, 0x73]);
  await session.sendPb(CAP_TRIGGER_SID, pb, 0x01, { ackTimeoutMs: 600, arm: TARGET_ARM as "L"|"R" }).catch(() => null);
}

// ---- main ----------------------------------------------------------------------
const outBase = process.argv[2] ?? `g2shot-${new Date().toISOString().replace(/[:.]/g, "-")}`;
const TARGET_ARM = (process.argv[3] ?? "R").toUpperCase();

const session = await G2Session.open();
console.log(`[shot] connected. capturing lens ${TARGET_ARM} via sid 0x${CAP_TRIGGER_SID.toString(16)}...`);
