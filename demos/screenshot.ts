...
function encodePngGray(px: Uint8Array, w: number, h: number): Uint8Array {
  const raw = new Uint8Array((w + 1) * h);       // one filter byte (0) per scanline
  for (let y = 0; y < h; y++) {
    raw[y * (w + 1)] = 0;
...
// ---- fragment reassembly -------------------------------------------------------
async function collectCapture(session: G2Session): Promise<Uint8Array> {
...
const CAP_TRIGGER_SID = 0x7d;
const CAP_OPCODE = 0xc7;
async function requestCapture(session: G2Session): Promise<void> {
  const pb = new Uint8Array([CAP_OPCODE]); // payload[0] == opcode; the CFW hook sees these raw bytes
  // The firmware sends no ack for an unknown serviceID, so the ack simply times out;
  // fragments are already being collected concurrently. Keep the timeout short.
  await session.sendPb(CAP_TRIGGER_SID, pb, 0x01, { ackTimeoutMs: 600, arm: "R" }).catch(() => null);
}
...
console.log(`[shot] connected. requesting capture on sid 0x${CAP_TRIGGER_SID.toString(16)}...`);

const capturePromise = collectCapture(session);
...
const { w, h, flags, px } = parseBlob(blob);
const png = encodePngGray(px, w, h);
const pgm = encodePgm(px, w, h);
const pngPath = `${outBase}.png`;
const pgmPath = `${outBase}.pgm`;
writeFileSync(pngPath, png);
writeFileSync(pgmPath, pgm);
console.log(`[shot] wrote ${pngPath} and ${pgmPath}  (${w}x${h} grayscale, up-filter=${flags & 1 ? "yes" : "no"})`);
