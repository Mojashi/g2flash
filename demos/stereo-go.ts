#!/usr/bin/env bun
// stereo-go.ts — arm-L ONLY. Load the clean build to both lenses, open, enable the barrier, confirm
// sync numerically, then turn on stereo ('d') and HOLD so you can watch the 3D fuse on the glasses.
...
async function pair() { R = null; L = null; await w(send([0x6b])); await w(send([0x6c])); await Bun.sleep(500); const d = R != null && L != null ? R - L : null; return `R=${R ?? "-"} L=${L ?? "-"} delta=${d ?? "?"}`; }

console.log(`clean build ${blob.length}B; ALL via arm L. reset + load...`);
...
await w(act(blob.length, crc)); await Bun.sleep(400); console.log(`  activate active=${act1}`);
console.log("open both ('g')..."); await w(send([0x67])); await Bun.sleep(1500);
console.log("baseline (no sync):"); for (let i = 0; i < 3; i++) console.log("  " + await pair());
console.log("barrier ON ('m'):"); await w(send([0x6d])); await Bun.sleep(1500);
for (let i = 0; i < 5; i++) console.log("  " + await pair());
console.log("\nSTEREO ON ('d') — WATCH THE GLASSES: the 5 squares should form a depth staircase, the");
console.log("border/title stay at screen depth, the marker sweeps L<->R in lockstep. Holding 20s...");
await w(send([0x64]));
for (let i = 0; i < 10; i++) { console.log("  " + await pair()); await Bun.sleep(1500); }
console.log("\n(leaving stereo on; 'n' to unsync, 'd' to flatten, 'q' to close via arm L)");
