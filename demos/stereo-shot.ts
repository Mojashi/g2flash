...
async function requestCapture(session: G2Session): Promise<void> {
  ...
  await session.sendPb(CAP_TRIGGER_SID, pb, 0x01, { ackTimeoutMs: 400, arm: "L" }).catch(() => null);
  await session.sendPb(CAP_TRIGGER_SID, pb, 0x02, { ackTimeoutMs: 400, arm: "R" }).catch(() => null);
}
