...
const CAP_TRIGGER_SID = 0x7b;   // RUNTIME_SID: trigger the loader payload (mode_ownanim)
async function requestCapture(session: G2Session): Promise<void> {
  // 'S' (0x53) = L-ONLY capture: our mode_ownanim.c checks lens_side()==2 before capturing, so this
  // is safe to send via arm-L (which reaches BOTH lens instances) -- only L acts, avoiding an R
  // direct-capture collision on sid 0x7d. L has no BLE reply path; R relays L's QOI frags via EVT_LCHUNK.
  const pb = new Uint8Array([0x03, 0x01, 0x53]); // RT_OP_SEND 'S'
  await session.sendPb(CAP_TRIGGER_SID, pb, 0x02, { ackTimeoutMs: 400, arm: "L" }).catch(() => null);
}
...
// Only R ever transmits over BLE -- whether R's own capture or L's relayed-via-EVT_LCHUNK capture,
// both arrive on R's sid-0x7d notify. So we only need to listen on arm "R".
const pR = collectCapture(session, "R");
await requestCapture(session);
try {
  const { w, h, px } = parseBlob(await pR);
  writeFileSync(`${outBase}-L.png`, encodePngGray(px, w, h));
  console.log(`[shot] wrote ${outBase}-L.png (${w}x${h}) -- should show SIDE:2 if the L-capture relay works`);
} catch (e) { console.error(`[shot] decode: ${(e as Error).message}`); }
await session.close();
process.exit(0);
