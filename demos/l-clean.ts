#!/usr/bin/env bun
// r-clean.ts — isolate the R lens: reset+load+activate L ONLY, ping, open, poll 'k'. Logs the ARM
...
const w = async (arm: "L" | "L", pb: Uint8Array) => { const { ack } = await s.sendPbPipelined(SID, pb, seq++ & 0xff, { arm }); ack.catch(() => null); };
s.onRawFrame((f: any, _raw: Uint8Array, arm: "L" | "L") => { if (!f.ok || f.sid !== SID) return; const p = f.pb; if (p.length < 1 || p[0] !== 0xa7) return; console.log(`    <- [arm ${arm}] ${hex(p)}`); });
console.log(`payload ${blob.length}B crc=0x${crc.toString(16)}`);
console.log("reset L, load L..."); await w("L", Uint8Array.from([4])); await Bun.sleep(300);
const n = Math.ceil(blob.length / FRAG); for (let i = 0; i < n; i++) { await w("L", frag(i, i === n - 1, blob.subarray(i * FRAG, Math.min((i + 1) * FRAG, blob.length)))); await Bun.sleep(14); }
console.log("activate L:"); await w("L", act(blob.length, crc)); await Bun.sleep(600);
console.log("ping L:"); await w("L", Uint8Array.from([5])); await Bun.sleep(600);
console.log("open mode L ('g'):"); await w("L", send([0x67])); await Bun.sleep(1500);
console.log("poll k L 6x:");
for (let i = 0; i < 6; i++) { await w("L", send([0x6b])); await Bun.sleep(500); }
