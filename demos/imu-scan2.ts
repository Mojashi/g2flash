#!/usr/bin/env bun
// imu-scan2.ts — load payload, send 'I' which scans 2 SRAM regions (headup_filter_ctx + ring_buf)
// for fields that changed over 500ms. Reports region/offset/value for the live ones.
...
console.log("scanning for live SRAM fields (500ms gap × 6 rounds, TILT YOUR HEAD)...\n");
for (let t = 0; t < 6; t++) {
  latest = null;
  await w(send([0x49]));
  await Bun.sleep(1200); // 500ms inside + BLE round-trip
  if (!latest || latest.length < 4) { console.log(`  round ${t}: no reply`); continue; }
  const cntA = latest[2]!, cntB = latest[3]!;
  let pos = 4;
  const entries: string[] = [];
  for (let i = 0; i < cntA + cntB; i++) {
    if (pos + 6 > latest.length) break;
    const reg = latest[pos]!, off = latest[pos + 1]!, val = u32at(latest, pos + 2);
    const fv = f32(val);
    entries.push(`${NAMES[reg]}+0x${off.toString(16).padStart(2, "0")}=${fv.toFixed(4)}(0x${val.toString(16)})`);
    pos += 6;
  }
  console.log(`  round ${t}: ${cntA}+${cntB} changed: ${entries.join("  ") || "(none)"}`);
}
