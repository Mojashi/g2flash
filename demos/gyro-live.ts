...
console.log("fresh load + open + imu...");
...
console.log("\ngyro cal float ×100 from current entry — TURN/NOD HEAD:");
console.log("  # flags  gx    gy    gz     accel_x accel_y accel_z");
for (let t = 0; t < 20; t++) {
  ...
  console.log(`  ${t.toString().padStart(2)} 0x${flags.toString(16).padStart(2,"0")} ${gx.toFixed(2).padStart(6)} ${gy.toFixed(2).padStart(6)} ${gz.toFixed(2).padStart(6)}   ${ax.toFixed(2).padStart(6)} ${ay.toFixed(2).padStart(6)} ${az.toFixed(2).padStart(6)}`);
}
