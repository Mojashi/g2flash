#!/usr/bin/env bun
// imu-probe.ts — load the new payload (arm-L), open, send 'I' to probe the IMU RAM globals,
// then rapidly poll the ring buffer to measure its internal update rate.
...
let probeReply: Uint8Array | null = null;
s.onRawFrame((f: any) => { if (!f.ok || f.sid !== SID) return; const p = f.pb;
  if (p.length >= 2 && p[0] === 0xa7 && p[1] === 0x49) probeReply = p; });
...
console.log("sending 'I' (IMU RAM probe)...");
probeReply = null;
await w(send([0x49])); await Bun.sleep(600);
if (!probeReply) { console.log("no probe reply"); await s.close(); process.exit(1); }
const labels = ["ring_buf_base", "headup_filter_ctx", "rate_config_table", "headup_ctx"];
for (let i = 0; i < 4; i++) {
  const ptr = u32at(probeReply, 2 + i * 8);
  const val = u32at(probeReply, 2 + i * 8 + 4);
  console.log(`  ${labels[i]}: ptr=0x${ptr.toString(16).padStart(8, "0")}  first_word=0x${val.toString(16).padStart(8, "0")}${ptr >= 0x20000000 ? " (SRAM)" : " (null/rodata)"}`);
}
