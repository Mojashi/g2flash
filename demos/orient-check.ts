...
console.log("orient ×100 (from 'I' probe) — TURN HEAD:");
for (let t = 0; t < 12; t++) {
  latest = null; await w(send([0x49])); await Bun.sleep(300);
  if (!latest || latest.length < 22) continue;
  const ox = i16at(latest,16)/100, oy = i16at(latest,18)/100, oz = i16at(latest,20)/100;
  const flags = latest[7];
  console.log(`  ${t}: orient=[${ox.toFixed(2)},${oy.toFixed(2)},${oz.toFixed(2)}] flags=0x${flags.toString(16)}`);
}
