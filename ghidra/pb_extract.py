...
json.dump({hex(a): {k: (v if not isinstance(v, list) else v) for k, v in m.items() if k != "fields"}
          ...
          open("/private/tmp/claude-501/-Users-mojashi-repos-odd/70e4d562-4b1b-41ba-9eac-bd869645bc38/scratchpad/pb_schema.json", "w"), indent=1)
...
for a in sorted(all_md, key=lambda x: (x not in roots, x)):
    ...
    out.append("}")
open("/private/tmp/claude-501/-Users-mojashi-repos-odd/70e4d562-4b1b-41ba-9eac-bd869645bc38/scratchpad/g2_schema.proto", "w").write("\n".join(out))
