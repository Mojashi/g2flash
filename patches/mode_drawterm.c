...
#define FW_DISPLAY_STARTUP 0x00443905u  /* display_startup(appID,payload,len): sanctioned foreground (Thumb) */
#define TERMINAL_APPID    0x30u
...
typedef int (*startup_fn)(unsigned appID, void* payload, unsigned len, unsigned x);
/* Foreground the (privileged) terminal app via the sanctioned display_startup path. This
 * demotes the dashboard so it stops compositing; terminal renders its page once then idles,
 * so a burst done AFTER persists. No code patching, no flush_cb freeze. */
static void take_foreground(ctx_t* c){
    ((startup_fn)FW_DISPLAY_STARTUP)(TERMINAL_APPID, 0, 0, 0);   /* async: queued to display_thread */
    { uint32_t t0 = c->api->tick_ms(); while (c->api->tick_ms() - t0 < 400u) { } }  /* let it foreground+render */
}
...
static void d_init(rt_api_t* api){ ctx_t* c = ctx_get(); if (c) take_foreground(c); draw_and_burst(api); }
...
static void d_exit(void){ ctx_t* c = ctx_get(); if (c){ c->api->mem_free(c); *(ctx_t* volatile*)MODE_CTX_SLOT = 0; } }
